...
Rust: dictionary.lookup("zhong guo") → Trie prefix match
  │
  ▼
Rust: candidate.generate() → [{text:"中国", freq:200}, ...]
  │
  ▼
...
```
+	zhong guo	中国	1700000000   # new entry
^	zhong guo	中国	1700000100   # frequency increment
```

Frequency decays exponentially with time (λ = 0.01, halves in ~70 days).
