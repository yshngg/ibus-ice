...
install:
	...
	install -m 644 engine/ibus_ice/__init__.py $(DESTDIR)$(PY_DIR)/__init__.py
	install -m 644 engine/ibus_ice/ffi.py $(DESTDIR)$(PY_DIR)/ffi.py
	install -m 644 engine/ibus_ice/engine.py $(DESTDIR)$(PY_DIR)/engine.py
	install -m 644 engine/ibus_ice/engine_main.py $(DESTDIR)$(PY_DIR)/engine_main.py
	install -m 755 engine/ibus-engine-ice.in $(DESTDIR)$(LIBEXEC_DIR)/ibus-engine-ice
	install -m 644 engine/ibus_ice/ice.xml $(DESTDIR)$(IBUS_COMPONENT_DIR)/ice.xml
...
uninstall:
	...
	rm -f $(DESTDIR)$(LIBEXEC_DIR)/ibus-engine-ice
